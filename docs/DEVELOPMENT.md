...
├── build/       # Compiled admin JS/CSS assets (generated by webpack.config.js).
├── build-apps/  # Compiled MCP app assets (generated by webpack.apps.config.js).
│
...
│
│   # Scripts and styles that are compiled by webpack.
├── src/
│   ├── admin/          # Admin UI (guide, connections, credentials).
│   ├── apps/           # Standalone MCP app pages.
...
- `@wordpress/element` — passes through to React
