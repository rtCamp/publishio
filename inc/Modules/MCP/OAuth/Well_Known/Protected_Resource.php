...
class Protected_Resource implements Registrable {
	...
	public function add_rewrite_rules(): void {
		...
		foreach ( Config::get_all_mcp_endpoint_paths() as $endpoint_path ) {
			add_rewrite_rule(
				...
				'index.php?rtpwai_oauth_wellknown=protected-resource&rtpwai_oauth_resource_path=' . rawurlencode( $endpoint_path ),
				'top'
			);
		}
		...
		add_rewrite_rule(
			...
			'index.php?rtpwai_oauth_wellknown=protected-resource',
			'top'
		);
	}
	...
	public function add_query_vars( array $vars ): array {
		$vars[] = 'rtpwai_oauth_wellknown';
		$vars[] = 'rtpwai_oauth_resource_path';
		return $vars;
	}
	...
	public function handle_request( \WP $wp ): void {
		if ( empty( $wp->query_vars['rtpwai_oauth_wellknown'] ) ) {
			return;
		}

		if ( 'protected-resource' !== $wp->query_vars['rtpwai_oauth_wellknown'] ) {
			return;
		}

		$resource_path = ! empty( $wp->query_vars['rtpwai_oauth_resource_path'] )
			? sanitize_text_field( $wp->query_vars['rtpwai_oauth_resource_path'] )
